...
# GrokMind Fusion — Voice Mode (LiveKit) with session logging + mobile-safe TTS
...
# Local deps
import tools  # grok_chat, livekit_token, n8n_post  (already in your repo)

# Session logger (robust import + fallback no-op)
try:
...
(async () => {{
  ...
  document.getElementById('startAudioBtn').onclick = () => {{
    try {{ const A = new Audio(); A.src = "data:audio/mp3;base64,//uQZAAAAAAAAAAAAAAAAAAAA"; A.play().catch(()=>{{}}); log("Start Audio gesture sent."); }}
...
def speak_in_browser(text: str):
    # Mobile-safe TTS: shows a small button the user must tap to unlock audio, then speaks.
...
def send_to_grok_and_show(prompt: str, speak: bool):
    ...
    try:
        ...
        # Best-effort n8n post (if you set N8N_LOG_URL inside session_log, it'll already be captured)
        try:
